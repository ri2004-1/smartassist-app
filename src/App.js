...
const STYLES = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .btn-primary {
    background: var(--accent);
    color: #fff;
    box-shadow: 0 4px 14px rgba(124,110,247,0.3);
  }

  .btn-primary:hover:not(:disabled) {
    background: #6a5ee0;
    box-shadow: 0 6px 20px rgba(124,110,247,0.45);
  ...
  .mic-btn.recording {
    ...
    box-shadow: 0 0 0 0 rgba(244,132,95,0.5);
    animation: pulse 1.2s infinite;
    color: #fff;
  }

  @keyframes pulse {
    0%   { box-shadow: 0 0 0 0   rgba(244,132,95,0.5); }
    70%  { box-shadow: 0 0 0 18px rgba(244,132,95,0); }
    100% { box-shadow: 0 0 0 0   rgba(244,132,95,0); }
  }
  ...
  return (
    <div className="panel">
      <textarea
        ...
        placeholder="Paste any text here — article, essay, notes, documentation…"
        value={text}
    ...
    <div className="panel">
      {!image ? (
        ...
        <div style={{position:"relative"}}>
          ...
          <button className="btn btn-danger" style={{position:"absolute",top:10,right:10,padding:"6px 12px",fontSize:12}} onClick={() => { setImage(null); setResult(""); }}>
            Remove
          </button>
        </div>
      )}
      ...
      <div className="tag-row">
        {QUESTIONS.map(q => (
          <div key={q} className={`tag ${prompt === q ? "active" : ""}`} style={prompt===q?{borderColor:"var(--accent)",color:"var(--accent)"}:{}} onClick={() => setPrompt(q)}>{q}</div>
        ))}
      </div>
      ...
      <div className="chat-input-row">
        ...
        />
        <button className="btn btn-primary" onClick={analyze} disabled={loading || !image}>
  ...
  return (
    <div className="panel">
      <div className="voice-center">
        {!supported && (
          <div className="result-box" style={{width:"100%",textAlign:"center"}}>
            ⚠️ Your browser doesn't support Speech Recognition.<br/>
            Try Chrome or Edge for the best experience.
          </div>
        )}

        <button
          className={`mic-btn ${status === "recording" ? "recording" : "idle"}`}
        ...
        <div className="row" style={{justifyContent:"center"}}>
          <button className="btn btn-primary" onClick={sendToAI} disabled={!transcript || loading}>
            {loading ? "Thinking…" : "Send to AI 🤖"}
          </button>
          {transcript && (
            <button className="btn btn-secondary" onClick={() => { setTranscript(""); setResult(""); setStatus("idle"); }}>
              Reset
            </button>
          )}
        </div>
      ...
      {result && (
        <div>
          <div className="result-box">
            <div className="result-label">AI Response (also spoken aloud)</div>
            {result}
          </div>
        </div>
      )}
